    ...
    func getWorkersWromBothTowns(){
        fetchWorkerDatafrom(workerUrl: tallinnWorkerList) { workerlist in
            for each in (workerlist?.employees)! {
                self.workerMergedArray.append(MainArray(fname: each.fname, lname: each.lname, position: each.position, email: each.contactDetails.email, phone: each.contactDetails.phone, projects: each.projects, phonebook: nil))
